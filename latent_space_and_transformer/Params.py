...
def ParseArgs():
	...
	parser.add_argument('--use_latent_space', action='store_true', help='whether to use latent space for diffusion')
	parser.add_argument('--latent_dim', type=int, default=128, help='dimension of latent space for diffusion')
	
	# Wandb run name
	parser.add_argument('--run_name', type=str, default=None, help='custom wandb run name (if not provided, will auto-generate)')
	
	# Transformer fusion parameters
	parser.add_argument('--use_transformer_fusion', action='store_true', help='whether to use transformer for multi-modal fusion')
	parser.add_argument('--transformer_layers', type=int, default=2, help='number of transformer layers for fusion')
	...
	parser.add_argument('--transformer_lr_scale', type=float, default=0.2, help='learning rate scale for transformer (relative to main LR)')
	parser.add_argument('--transformer_grad_clip', type=float, default=1.0, help='gradient clipping threshold for transformer')
	parser.add_argument('--transformer_mix_weight', type=float, default=0.5, help='weight for mixing transformer output with weighted sum (0=only weighted sum, 1=only transformer)')
	parser.add_argument('--transformer_align_weight', type=float, default=0.1, help='weight for alignment loss between transformer output and weighted sum')
