...
class GaussianDiffusion(nn.Module):
	def __init__(self, noise_scale, noise_min, noise_max, steps, beta_fixed=True):
        ...
        # continuous-time helpers (for ct mode)
		self.var_start = self.noise_scale * self.noise_min
		self.var_end = self.noise_scale * self.noise_max
	...
	# ----------- continuous schedule (new) -----------
	def alpha_bar_cont(self, t):
		"""
		Continuous extension of the original linear variance idea.
		t in [0, 1]
		"""
		# If noise_scale = 0, treat as no-noise identity
		if self.noise_scale == 0:
			return torch.ones_like(t)

		var = self.var_start + (self.var_end - self.var_start) * t
		a_bar = 1.0 - var
		return torch.clamp(a_bar, 1e-5, 1.0)

	def q_sample_cont(self, x_start, t, noise=None):
		if noise is None:
			noise = torch.randn_like(x_start)
		a_bar = self.alpha_bar_cont(t).float()
		while len(a_bar.shape) < len(x_start.shape):
			a_bar = a_bar[..., None]
		return torch.sqrt(a_bar) * x_start + torch.sqrt(1.0 - a_bar) * noise

	def SNR_cont(self, t):
		a_bar = self.alpha_bar_cont(t)
		return a_bar / (1.0 - a_bar)
#####################################.####################
	...
	def p_sample_ct_ode(self, model, x_start, steps, ode_nfe=20, sampling_noise=False):
		"""
		ODE/DDIM-style deterministic fast sampler by skipping discrete timesteps.
		This is a lightweight approximation that reduces the number of denoise evaluations.

		Args:
			model: Denoise network
			x_start: [B, item] one-hot/soft interaction vector
			steps: warm-start steps for q_sample (same semantics as p_sample)
			ode_nfe: number of function evaluations (effective denoise steps)
			sampling_noise: kept for API compatibility; recommended False for ODE mode
		"""
		# warm start
		if steps == 0:
			x_t = x_start
		...
		# choose a reduced set of timesteps
		nfe = max(1, int(ode_nfe))
		# linspace may produce duplicates after int-cast; keep unique descending
		ts = np.linspace(self.steps - 1, 0, num=nfe, dtype=np.int64)
		ts = sorted(list(set(ts.tolist())), reverse=True)
		...
		for i in ts:
			...
			if sampling_noise:
				# not recommended for ODE mode, but keep consistent behavior
				noise = torch.randn_like(x_t)
				nonzero_mask = ((t != 0).float().view(-1, *([1] * (len(x_t.shape) - 1))))
				x_t = model_mean + nonzero_mask * torch.exp(0.5 * model_log_variance) * noise
			else:
    ...
    # ----------- continuous deterministic sampling (new) -----------
	def _scale_time_for_model(self, t):
		"""
		Map continuous t to the time domain expected by the denoiser embedding.
		"""
		if args.ode_time_scale == 'discrete':
			# map [0,1] -> [0, steps-1]
			return t * max(self.steps - 1, 1)
		else:
			return t

	def p_sample_ct_ode_continuous(self, model, x_start):
		"""
		Continuous-time deterministic sampler.
		Implemented as DDIM-style update using alpha_bar_cont.
		"""
		B = x_start.size(0)
		if self.noise_scale == 0:
			return x_start

		# start from noisy state at t=1
		t1 = torch.ones(B, device=x_start.device)
		x_t = self.q_sample_cont(x_start, t1)

		# time grid from 1 -> 0
		ts = torch.linspace(1.0, 0.0, args.ode_nfe + 1, device=x_start.device)

		for k in range(args.ode_nfe):
			t = ts[k].repeat(B)
			s = ts[k + 1].repeat(B)

			a_t = self.alpha_bar_cont(t).float()
			a_s = self.alpha_bar_cont(s).float()

			while len(a_t.shape) < len(x_t.shape):
				a_t = a_t[..., None]
				a_s = a_s[..., None]

			t_in = self._scale_time_for_model(t)

			# predict x0
			x0_hat = model(x_t, t_in, False)

			# infer eps
			eps_hat = (x_t - torch.sqrt(a_t) * x0_hat) / torch.sqrt(1.0 - a_t)

			# deterministic DDIM/ODE-like update
			x_t = torch.sqrt(a_s) * x0_hat + torch.sqrt(1.0 - a_s) * eps_hat
...
######################################################################
	...
	# 	return diff_loss, gc_loss
	    # ----------- training loss (discrete + continuous switch) -----------
	def training_losses(self, model, x_start, itmEmbeds, batch_index, model_feats):
		batch_size = x_start.size(0)
		noise = torch.randn_like(x_start)
		...
		if args.use_ct_ode:
			...
			if self.noise_scale != 0:
				x_t = self.q_sample_cont(x_start, t, noise)
			else:
				x_t = x_start

			t_in = self._scale_time_for_model(t)

			model_output = model(x_t, t_in)

			# simplest stable choice for continuous training
			mse = self.mean_flat((x_start - model_output) ** 2)
			diff_loss = mse
		else:
			ts = torch.randint(0, self.steps, (batch_size,)).long().cuda()
