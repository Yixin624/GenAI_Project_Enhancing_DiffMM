...
def ParseArgs():
    ...
    # ---------- continuous-time ODE switch (new) ----------
	parser.add_argument('--use_ct_ode', action='store_true',
						help='Use continuous-time diffusion training and ODE/DDIM-style deterministic sampling')

	parser.add_argument('--ode_nfe', type=int, default=20,
						help='Number of continuous-time steps for ODE/DDIM sampling')

	# time input scaling for denoiser embedding
	# "discrete": map t in [0,1] -> [0, steps-1] for compatibility
	# "unit": use t directly in [0,1]
	parser.add_argument('--ode_time_scale', type=str, default='discrete',
						choices=['discrete', 'unit'])
	parser.add_argument('--wandb_project', type=str, default='enhancing_diffmm')
